...
const Client = require('../clients-old/ClientsModel');
const Visit = require('./VisitsModel');

class VisitService {
  async getVisitList(clientId) {
    try {
      const client = await Client.findById(clientId).select('visits._id visits.date visits.time visits.done').exec();
